    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                print("{}+1,已有".format(word) + str(self.__counter) + "张{}".format(word))
                self.__counter += 1
        return
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        #crawler.start('逆光剪影', 10, 2, 30)
        #crawler.start('日出日落', 10, 1, 30)
        #crawler.start('秋天树叶', 10, 1, 30)
        #crawler.start('QR二维码',10,1,30)
        #crawler.start('烟花', 10, 1, 30)
        #crawler.start('霓虹灯',10, 1, 30)
        #crawler.start('蓝天',10, 1, 30)
        #crawler.start('阴天', 10, 1, 30)
        #crawler.start('演出现场', 10, 1, 30)
        #crawler.start('花', 10, 1, 30)
        #crawler.start('美食', 10, 1, 30)
        #crawler.start('微距', 10, 1, 30)
        #crawler.start('海滩', 10, 1, 30)
        #crawler.start('烛光', 10, 1, 30)
        #crawler.start('夜景', 10, 1, 30)
        #crawler.start('山', 20, 1, 30)
        #crawler.start('雪景', 10, 2, 30)
        #crawler.start('瀑布', 10, 1, 30)
        #crawler.start('风景田园', 10, 1, 30)
        #crawler.start('绿色植物', 10, 1, 30)
        #crawler.start('扫描文件', 10, 1, 30)
        #crawler.start('绿色树林', 10, 1, 30)
        #crawler.start('幼儿', 5, 1, 30)
        crawler.start('portrait', 10, 1, 30)
